...
// ══════════════ CYBERSHIELD GREEN/BLACK DEFAULT OPTIONS ══════════════
const defaultOptions: HyperspeedOptions = {
  ...
  colors: {
    ...
    // CyberShield green neon palette
    leftCars: [0x00FF41, 0x00CC33, 0x33FF66],
    rightCars: [0x00AA22, 0x006611, 0x003300],
    sticks: 0x00FF41
  }
};
...
class LightsSticks {
  ...
  init() {
    ...
    if (Array.isArray(options.colors.sticks)) {
      colorArray = options.colors.sticks.map((c: number) => new THREE.Color(c));
    } else {
...
class App {
  ...
  update(delta: number) {
    ...
    const scaledDelta = delta * speedFactor;
    const lerpPercentage = Math.exp(-(-60 * Math.log2(1 - 0.1)) * scaledDelta);
    this.speedUp += lerp(this.speedUp, this.speedUpTarget, lerpPercentage, 0.00001);
    this.timeOffset += this.speedUp * scaledDelta;
    const time = (this.clock.elapsedTime * speedFactor) + this.timeOffset;
    ...
    if (fovChange !== 0) {
      this.camera.fov += fovChange * scaledDelta * 6;
      updateCamera = true;
    }
...
// ══════════════ CYBERSHIELD PRESET ══════════════
export const cyberShieldPreset: Partial<HyperspeedOptions> = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
  distortion: 'turbulentDistortion',
  length: 400,
  ...
  colors: {
    ...
    brokenLines: 0x004411,
    // CyberShield green neon palette — BRIGHT
    leftCars: [0x00FF41, 0x39FF14, 0x66FF66],
    rightCars: [0x00CC33, 0x00AA22, 0x00FF41],
    sticks: 0x00FF41
  }
};

export const offensivePreset: Partial<HyperspeedOptions> = {
  ...cyberShieldPreset,
  colors: {
    roadColor: 0x050505,
    islandColor: 0x080808,
    ...
    brokenLines: 0x440000,
    leftCars: [0xff1e1e, 0xff3b30, 0xbf0909],
    rightCars: [0xff5b5b, 0xff3b30, 0xff0000],
    sticks: 0xff3b30
  }
};

export const defensivePreset: Partial<HyperspeedOptions> = {
  ...cyberShieldPreset,
  colors: {
    roadColor: 0x050505,
    islandColor: 0x080808,
    background: 0x000000,
    shoulderLines: 0x00c7ff,
    brokenLines: 0x003344,
    leftCars: [0x00c7ff, 0x00a2ff, 0x0055aa],
    rightCars: [0x5bceff, 0x00c7ff, 0x0077ff],
    sticks: 0x00c7ff
  }
};
